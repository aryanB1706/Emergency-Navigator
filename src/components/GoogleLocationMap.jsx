...
export default function HospitalMap() {
  ...
  const hospitalIcon = useMemo(() => {
    ...
    return {
      url: "data:image/svg+xml;charset=UTF-8," +
           encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24" viewBox="0 0 24 24" fill="#EA4335"><path d="M19 3H5c-1.1 0-1.99.9-1.99 2L3 19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-1 11h-4v4h-4v-4H6v-4h4V6h4v4h4v4z"/></svg>'),
  ...
  const selectedHospitalIcon = useMemo(() => {
    ...
    return {
      url: "data:image/svg+xml;charset=UTF-8," +
           encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24" viewBox="0 0 24 24" fill="#34A853"><path d="M19 3H5c-1.1 0-1.99.9-1.99 2L3 19c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-1 11h-4v4h-4v-4H6v-4h4V6h4v4h4v4z"/></svg>'),
        ...
        try {
          ...
          const response = await fetch(
            `https://places.googleapis.com/v1/places:searchText`,
            {
              ...
              body: JSON.stringify({ textQuery: "hospital", locationBias: { circle: { center: { latitude: userPosition.lat, longitude: userPosition.lng }, radius } }, maxResultCount: 15, languageCode: "en", rankPreference: "DISTANCE" }),
            }
          );
  ...
  const calculateRoute = useCallback(async (hospital) => {
      if (!userPosition || !isLoaded) return;
      const origin = livePosition || userPosition;
      const directionsService = new window.google.maps.DirectionsService();
      directionsService.route(
        { origin, destination: { lat: hospital.location.latitude, lng: hospital.location.longitude }, travelMode: window.google.maps.TravelMode.DRIVING },
        async (result, status) => {
          if (status === "OK" && result) {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-[90vh] bg-gray-100 p-2 rounded-xl gap-2">
        ...
        >
          ...
          {hospitals.map((hospital) => (
            <MarkerF
              ...
              icon={selectedHospital?.location.latitude === hospital.location.latitude ? selectedHospitalIcon : hospitalIcon}
            />
          ))}
          {selectedHospital && (
            <InfoWindowF position={{ lat: selectedHospital.location.latitude, lng: selectedHospital.location.longitude }} onCloseClick={() => setSelectedHospital(null)}>
      ...
      <div className="w-full md:w-1/3 h-1/2 md:h-full overflow-y-auto bg-gray-50 p-4 rounded-xl shadow">
        ...
        {loadingHospitals ? (<p>Searching hospitals...</p>) : hospitals.length > 0 ? (
          hospitals.map((hospital, idx) => (
            <Card key={idx} className="mb-3 cursor-pointer shadow hover:shadow-lg transition" onClick={() => calculateRoute(hospital)}>
              <CardContent>
